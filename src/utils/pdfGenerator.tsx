...
const styles = StyleSheet.create({
  ...
  introSheet: {
    ...
    maxHeight: "98%",
    borderWidth: 2,
    borderColor: "rgba(28, 59, 90, 0.08)",
    borderStyle: "solid",
  },
  introTitle: {
    fontSize: 26,
...
const getContent = (battery: string, level: string, field: string): string => {
  ...
  try {
    const content = (pdfContentData as any)[frenchBattery]?.[frenchLevel]?.[
      field
    ];
    return content || "";
  } catch {
...
const getAccentColor = (level: string): string => {
  switch (level) {
    case "critical":
      return "#D96536"; // terracotta
    case "unstable":
      return "#DEA742"; // amber
    case "optimal":
      return "#379191"; // sarcelle
    default:
      return "#1c3b5a";
  }
};
...
const getLightBg = (level: string): string => {
  switch (level) {
    case "critical":
      return "rgba(217, 101, 54, 0.12)";
    case "unstable":
      return "rgba(222, 167, 66, 0.12)";
    case "optimal":
      return "rgba(55, 145, 145, 0.12)";
    default:
      return "rgba(28, 59, 90, 0.12)";
  }
};
...
const BatteryDetailPage = ({
  ...
  // Get content
  const headerSubtitle = getContent(
  ...
  const vivis = [1, 2, 3, 4, 5, 6]
    .map((i) =>
      getContent(batteryScore.battery, batteryScore.level, `S1_VIVIS_${i}`)
    )
  ...
  const tedis = [1, 2, 3, 4, 5, 6]
    .map((i) =>
      getContent(batteryScore.battery, batteryScore.level, `S1_TEDIS_${i}`)
    )
    .filter(isValidContent);

  // S2_SCI - Scientific explanation
  const science1 = getContent(
    batteryScore.battery,
    batteryScore.level,
    "S2_SCI_1"
  );
  const scienceExplain1 = getContent(
    batteryScore.battery,
    batteryScore.level,
    "SCIENCE_1_EXPLAIN"
  );
  const science2 = getContent(
    batteryScore.battery,
    batteryScore.level,
    "S2_SCI_2"
  );
  const scienceExplain2 = getContent(
    batteryScore.battery,
    batteryScore.level,
    "SCIENCE_2_EXPLAIN"
  );
  ...
  const exoIntro = getContent(
    batteryScore.battery,
    batteryScore.level,
    "EXO_INTRO"
  );
  const exoDuree = getContent(
    batteryScore.battery,
    batteryScore.level,
    "S3_EXO_DUREE"
  );
  const exoMateriel = getContent(
    batteryScore.battery,
    batteryScore.level,
    "S3_EXO_MATERIEL"
  );
  const exoSteps = [1, 2, 3, 4, 5, 6]
    .map((i) =>
      getContent(batteryScore.battery, batteryScore.level, `S3_EXO_ETAPE_${i}`)
    )
    .filter(isValidContent);
  const exoCloture = getContent(
    batteryScore.battery,
    batteryScore.level,
    "EXO_CLOTURE"
  );
  const exoPourquoi = getContent(
    batteryScore.battery,
    batteryScore.level,
    "S3_EXO_POURQUOI"
  );
  ...
  const reco1Items = ["A", "B", "C"]
    .map((l) =>
      getContent(batteryScore.battery, batteryScore.level, `S4_RECO1_${l}`)
    )
    .filter(isValidContent);

  const reco2Title = getContent(
    batteryScore.battery,
    batteryScore.level,
    "S4_RECO2_TITRE"
  );
  const reco2Items = ["A", "B", "C"]
    .map((l) =>
      getContent(batteryScore.battery, batteryScore.level, `S4_RECO2_${l}`)
    )
    .filter(isValidContent);

  // CTA and Footer
  const ctaLocal = getContent(
    batteryScore.battery,
    batteryScore.level,
    "CTA_LOCAL"
  );
  const footerForceCta = getContent(
    batteryScore.battery,
    batteryScore.level,
    "FOOTER_FORCE_CTA"
  );
          ...
          <View style={styles.mainContent}>
            ...
            {vivis.length > 0 && (
              <View style={styles.section}>
                <Text
                  style={[styles.sectionHeader, { backgroundColor: lightBg }]}
                >
                  1. Ce que tu vis
                </Text>
              ...
              <View style={styles.section}>
                ...
                <View style={styles.sectionBody}>
                  ...
                  {isValidContent(science2) && (
                    <>
                      <Text style={[styles.bulletPoint, { marginTop: 2 }]}>
                        • {science2}
                      </Text>
            ...
            {exoTitle && (
              <View style={styles.section}>
                ...
                <View
                  style={[
                    styles.exerciseBox,
                    { borderLeftColor: accentColor, backgroundColor: lightBg },
                  ]}
                >
                  ...
                  {exoSteps.map((step, i) => (
                    <Text key={i} style={styles.exerciseStep}>
                      • {step}
                    </Text>
                  ))}
                  {exoPourquoi && isValidContent(exoPourquoi) && (
                    <Text style={styles.exerciseWhy}>
                      Pourquoi ça marche : {exoPourquoi}
                    </Text>
                  )}
                  {exoCloture && isValidContent(exoCloture) && (
                    <Text style={[styles.sectionBody, { marginTop: 3 }]}>
                      {exoCloture}
                    </Text>
                  )}
                </View>
                  ...
                  {reco1Title && (
                    <View style={styles.recoBox}>
                      ...
                      {reco1Items.map((item, i) => (
                        <Text key={i} style={styles.recoItem}>
                          • {item}
                        </Text>
                      ))}
                    </View>
        ...
        {footerForceCta && isValidContent(footerForceCta) && (
          <View
            style={{
              marginTop: 4,
              padding: 5,
              backgroundColor: "rgba(28, 59, 90, 0.06)",
              borderRadius: 6,
            }}
          >
            <Text
    ...
    <Page size="A4" style={styles.page}>
      <View style={styles.introSheet}>
        ...
        <View style={styles.introImageContainer}>
          <Image src="/ayoub-sadry.png" style={styles.introImage} />
        </View>
        ...
        <View
          style={{
            width: "100%",
            height: 1,
            backgroundColor: "rgba(28, 59, 90, 0.1)",
            marginVertical: 15,
          }}
        />
        ...
        <View>
          ...
          <Text style={{ ...styles.introParagraph, marginBottom: 0 }}>
            ...
            intérieur, en reliant neurosciences, conscience et actions simples.
          </Text>
        </View>
        ...
        >
          ...
          <Text style={{ fontWeight: "bold" }}>vivant(e)</Text>,{"\n"}pas juste
          performant(e).
        </Text>
...
const DiagnosticOverviewPage = ({
  ...
  const getBurnoutState = () => {
    if (results.totalPercentage >= 80)
      return { label: "Énergie Optimale", color: "#379191" };
    if (results.totalPercentage >= 60)
      return { label: "Fatigue Modérée", color: "#dea742" };
    if (results.totalPercentage >= 40)
      return { label: "Épuisement Installé", color: "#d96536" };
    return { label: "Burn-Out Silencieux", color: "#c63f2a" };
  };
    ...
    <Page size="A4" style={styles.page}>
      <View style={styles.sheet}>
        ...
        >
          ...
          >
            <View>
              <Text style={{ fontSize: 10, color: "#7e8081", marginBottom: 3 }}>
                État Général
              </Text>
              <Text
                style={{
        ...
        {/* Septogram Visual Grid - Energy Wheel */}
        <View
          style={{
            padding: 15,
        ...
        >
          ...
          >
            {/* NIIA Power Core - Enhanced center hub */}
            <View
              style={{
                width: 50,
                height: 50,
              ...
              {results.scores.map((score, index) => {
                ...
                return (
                  <View key={score.battery}>
                    {/* Energy Spoke - Power line from NIIA core to battery */}
                    <View
                      style={{
                        position: "absolute",
                        left: spokeStartX,
                    ...
                    >
                      {/* Battery container with energy glow effect */}
                      <View
                        style={{
                          width: 34,
                          height: 47,
                        ...
                        {[4, 3, 2, 1].map((seg) => (
                          <View
                            ...
                            style={{
                              ...
                              marginBottom: 1,
                              backgroundColor:
                                seg <= segments ? color : "#f0f0f0",
                              opacity: seg <= segments ? 0.9 : 0.3,
                            }}
                          />
                        ))}
                      </View>
        ...
        <View
          style={{
            ...
            paddingTop: 8,
            borderTopWidth: 1,
            borderTopColor: "rgba(130,114,104,0.2)",
      ...
      <Page size="A4" style={styles.page}>
        ...
        >
          ...
          >
            ...
            >
              Comment transformer ton épuisement silencieux en clarté intérieure
              et pouvoir personnel grâce à la méthode IMPACT
            </Text>
            ...
            <View style={{ marginBottom: 15 }}>
              ...
              >
                • Pourquoi les pauses ne guérissent jamais le burn-out
                silencieux
              </Text>
              <Text
                style={{
              ...
              >
                • Pourquoi ton intelligence est ton atout pour sortir de
                l'épuisement
              </Text>
              <Text
                style={{
