...
const Questionnaire = ({ onComplete }: Props) => {
  ...
  const [currentBattery, setCurrentBattery] = useState<BatteryType | null>(null);
  const [showBatteryValidation, setShowBatteryValidation] = useState(false);
  const [completedBattery, setCompletedBattery] = useState<BatteryType | null>(null);
  ...
  const getBatteryMotivationalMessage = (battery: BatteryType): string => {
    const messages: Record<BatteryType, string> = {
      physical: "Bravo ! Tu viens d'explorer ta batterie physique. Continue, ton corps te remerciera ! 💪",
      mental: "Excellent ! Ta batterie mentale est évaluée. Chaque réponse te rapproche de la clarté ! 🧠",
      emotional: "Magnifique ! Tu as pris le temps d'explorer tes émotions. Continue ce beau travail ! 💙",
      identity: "Superbe ! Ta batterie identitaire est complète. Tu te découvres un peu plus ! ✨",
      relational: "Fantastique ! Tes relations sont maintenant cartographiées. Continue sur ta lancée ! 🤝",
      professional: "Incroyable ! Ta batterie professionnelle est évaluée. Ton avenir te dit merci ! 💼",
      spiritual: "Merveilleux ! Tu as exploré ta dimension spirituelle. La dernière étape approche ! 🕊️"
    };
  ...
  const isAnswered = currentQuestion >= 0 && answers[questions[currentQuestion]?.id] !== undefined;
          ...
          <CardContent className="space-y-6">
            <div className="flex flex-wrap gap-2 justify-center">
              {getAllBatteries().map(battery => {
                ...
                return (
                  <Badge key={battery} variant="outline" className="gap-2 px-3 py-1.5">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && firstName.trim() && lastName.trim() && email.trim() && handleStart()}
                  className="text-base"
                />
              </div>
      ...
      <div className="h-screen flex flex-col bg-gradient-to-br from-niia-beige-light via-white to-niia-beige">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          ...
          <CardContent className="pt-8 text-center space-y-6">
            <div>
              <h2 className="text-2xl font-spartan text-niia-blue-dark mb-2">Batterie {batteryData.name} complétée !</h2>
              <p className="text-lg text-niia-gray font-medium mt-4 px-4">{motivationalMessage}</p>
            </div>
            ...
            <div className="space-y-2">
              <p className="text-lg font-semibold">
                {completedCount === totalBatteries 
                  ...
                  : "Excellent ! Tu es un pas de plus vers la clarté."}
              </p>
              <p className="text-sm text-muted-foreground">
                {completedCount === totalBatteries 
                  ? "Ton rapport personnalisé t'attend..." 
                  : `Plus que ${totalBatteries - completedCount} ${totalBatteries - completedCount === 1 ? 'batterie' : 'batteries'} à évaluer.`}
              </p>
            </div>

            <Button onClick={handleNext} className="w-full bg-niia-blue-dark hover:bg-niia-teal-dark text-white font-semibold" size="lg">
              {completedCount === totalBatteries ? 'Voir mes résultats' : 'Continuer'}
              <ChevronRight className="ml-2" size={20} />
            </Button>
          </CardContent>
        </Card>
  ...
  // Question screen
  const question = questions[currentQuestion];
  const batteryColor = currentBattery ? batteryInfo[currentBattery].color : '#666';
  const batteryQuestions = currentBattery ? getBatteryQuestions(currentBattery) : [];
  const currentBatteryQuestionIndex = batteryQuestions.findIndex(q => q.id === question.id) + 1;
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-niia-beige-light via-white to-niia-beige">
      <Header 
        ...
        batteryName={currentBattery ? `Batterie ${batteryInfo[currentBattery].name} (${currentBatteryQuestionIndex}/${batteryQuestions.length})` : undefined}
      />
        ...
        <div className="w-full max-w-3xl h-full flex flex-col">
              ...
              {question.options.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    style={isSelected ? { borderColor: batteryColor, backgroundColor: `${batteryColor}10` } : {}}
                  >
                    <div className="flex items-start gap-2">
                      <div
                        ...
                        style={isSelected ? { borderColor: batteryColor, backgroundColor: batteryColor } : {}}
                      >
                        {isSelected && <CheckCircle size={10} className="text-white" />}
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-medium">{option.text}</p>
                        <p className="text-xs text-muted-foreground italic mt-0.5">{option.example}</p>
                      </div>
                    </div>
                  </button>
                );
          ...
          <div className="flex gap-3 mt-4">
            ...
            <Button
              ...
              disabled={!isAnswered}
              size="default"
              className={`flex-1 font-semibold ${isAnswered ? 'bg-niia-blue-dark hover:bg-niia-teal-dark text-white' : ''}`}
            >
              {currentQuestion === totalQuestions - 1 ? 'Voir mes résultats' : 'Suivant'}
